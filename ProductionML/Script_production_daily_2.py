...
# HANYA untuk libur panjang yang benar-benar shutdown 100%.
# Libur singkat (Isra Miraj, Imlek, dll) tetap pakai HOLIDAY_FACTOR (0.15x)
# karena pabrik masih beroperasi sebagian (terutama SHIFT2).
...
# Cukup tambah entry baru saat tahun baru → otomatis adaptive
RAMADAN_START_MONTHS = {
    ...
    2026: (2026, 2),  # Ramadan mulai Feb 2026
    2027: (2027, 2),  # Ramadan mulai Feb 2027
}

print("=" * 70)
print("SCRIPT_FIX_DAILY v2.2 — Layer 2 Improved (Task 1-4 + Adaptive Weekend)")
print("=" * 70)
...
def _get_adaptive_weekend_boost(df_hist, target_year, target_month):
    """
    [FIX Opsi D] Adaptive Pre-Ramadan Weekend Profiler.
    
    Menghitung boost Sabtu dan Minggu secara TERPISAH dari rata-rata
    bulan pre-Ramadan tahun-tahun sebelumnya. Self-correcting karena
    semakin banyak tahun data, semakin akurat.
    
    Logika:
    1. Cari semua bulan pre-Ramadan historis SEBELUM target (zero leakage)
    2. Untuk setiap bulan pre-Ramadan, hitung Sat/WD dan Sun/WD ratio
    3. Untuk periode normal (non-pre-Ramadan), hitung Sat/WD dan Sun/WD ratio
    4. Boost = rata-rata(pre-Ramadan ratio) / rata-rata(normal ratio)
    
    Returns: (sat_boost, sun_boost) atau None jika tidak pre-Ramadan
    """
    ...
    daily = df_clean.groupby(["tanggal", "day_of_week", "is_weekend"])["demand"].sum().reset_index()
    ...
    for ry, (ram_y, ram_m) in RAMADAN_START_MONTHS.items():
        ...
        # Hanya gunakan data SEBELUM target year/month (zero leakage)
        if (pre_y > target_year) or (pre_y == target_year and pre_m >= target_month):
            continue
        
        # Ambil data bulan pre-Ramadan
        ...
        if not sat_data.empty:
            pre_ram_sat_ratios.append(sat_data["demand"].mean() / wd_avg)
        if not sun_data.empty:
            pre_ram_sun_ratios.append(sun_data["demand"].mean() / wd_avg)
    
    if not pre_ram_sat_ratios or not pre_ram_sun_ratios:
        return None  # Tidak cukup data historis
    
    avg_pre_sat = np.mean(pre_ram_sat_ratios)
    avg_pre_sun = np.mean(pre_ram_sun_ratios)
    
    return avg_pre_sat, avg_pre_sun
...
def build_dow_share_profile(df_hist, target_year, target_month, window_months=DOW_PROFILE_WINDOW_MONTHS):
    ...
    [FIX Opsi D] Adaptive Pre-Ramadan Weekend Profiler:
    Bulan sebelum Ramadan → weekend ratio diambil langsung dari rata-rata
    bulan pre-Ramadan historis (bukan hardcoded boost). Self-correcting
    saat data tahun baru ditambahkan.
    ...
    adaptive_we = _get_adaptive_weekend_boost(df_hist, target_year, target_month)
    if adaptive_we is not None:
        ...
        print(f"    🔺 Pre-Ramadan ADAPTIVE weekend profile:")
...
try:
    ...
    # [FIX] TIDAK lagi auto-add semua all-shift-inactive ke FACTORY_SHUTDOWN_DATES.
    # Libur singkat (Isra Miraj, Imlek) punya SQL all-shift-inactive tapi pabrik
    # tetap beroperasi sebagian. Biarkan ditangani oleh HOLIDAY_FACTOR (0.15x).
    # FACTORY_SHUTDOWN_DATES hanya diisi secara EKSPLISIT di config (Lebaran + NYE).

    _calendar_source = "SQL Server"
    print(f"  ✅ Kalender dibaca dari SQL Server")
    print(f"     Holidays dari SQL: {len(LIB_HOLIDAYS)} hari")
    print(f"     Shutdown dates (hardcoded only): {len(FACTORY_SHUTDOWN_DATES)} hari")
...
for month_str in FORWARD_MONTHS:
    ...
    # Inisialisasi library holidays untuk deteksi jenis libur (Smart Classifier)
    import holidays
    ...
    def get_day_weight_hibrida(row):
        ...
        # 1. Deteksi Shutdown berdasarkan keyword di holidays library
        ...
        # Prioritas Tertinggi: Shutdown
        if dt in FACTORY_SHUTDOWN_DATES or is_shutdown:
            return 0.0
            
        # 2. Deteksi Hangover / Cuti Bersama
        is_hangover = False
        if hol_name and ("Cuti Bersama" in hol_name or "Joint Leave" in hol_name):
            is_hangover = True
        else:
            # Cek apakah H-1 atau H-2 adalah Shutdown (Hangover effect)
            ...
            # Jika dia nempel shutdown DAN statusnya libur (atau akhir pekan libur)
        ...
        # Tier 2: Holiday terpisah berdasarkan kelas
        if is_hangover:
            return weekday_avg * 0.08  # ~250 unit
        if is_hol:
            return weekday_avg * 0.24  # Standalone ~750 unit
...
# ══════════════════════════════════════════════════════════════════════
# STEP 7B: DAILY ERROR PROFILE (Section 2B)
# ══════════════════════════════════════════════════════════════════════
if _has_actual:
    daily_pred = df_cmp.groupby("tanggal")["demand_pred_int"].sum()
    daily_act  = df_cmp.groupby("tanggal")["actual"].sum()
    daily_df   = pd.DataFrame({"pred": daily_pred, "actual": daily_act}).reset_index()
    daily_df   = daily_df[daily_df["actual"] > 0]
    daily_df["err_pct"] = (daily_df["pred"] - daily_df["actual"]) / daily_df["actual"] * 100
    daily_df["abs_err"] = daily_df["err_pct"].abs()

    # MADE & WAPE
    MADE = daily_df["abs_err"].mean()
    WAPE = (daily_df["pred"] - daily_df["actual"]).abs().sum() / daily_df["actual"].sum() * 100

    # Exclude 2 outlier (1 Jan + 3 Jan)
    outlier_dates = {pd.Timestamp("2026-01-01"), pd.Timestamp("2026-01-03")}
    daily_excl = daily_df[~daily_df["tanggal"].isin(outlier_dates)]
    MADE_excl  = daily_excl["abs_err"].mean()
    WAPE_excl  = (daily_excl["pred"] - daily_excl["actual"]).abs().sum() / daily_excl["actual"].sum() * 100

    # Weekday vs Weekend
    daily_df["is_weekend"] = daily_df["tanggal"].dt.weekday >= 5
    df_wd = daily_df[~daily_df["is_weekend"]]  # ← tambah ini
    df_we = daily_df[daily_df["is_weekend"]]   # ← tambah ini

    MADE_wd = df_wd["abs_err"].mean()
    MADE_we = df_we["abs_err"].mean()
    WAPE_wd = (df_wd["pred"] - df_wd["actual"]).abs().sum() / df_wd["actual"].sum() * 100
    WAPE_we = (df_we["pred"] - df_we["actual"]).abs().sum() / df_we["actual"].sum() * 100

    # Worst days
    worst5 = daily_df.nlargest(5, "abs_err")[["tanggal","pred","actual","err_pct"]]

    print(f"\n{'='*60}")
    print(f"  SECTION 2B — DAILY ERROR PROFILE")
    print(f"{'='*60}")
    print(f"  MADE  (full)       : {MADE:>6.1f}%  {'✅' if MADE<15 else '❌'} (threshold 15%)")
    print(f"  MADE  (excl outlier): {MADE_excl:>6.1f}%  {'✅' if MADE_excl<15 else '❌'}")
    print(f"  WAPE  (full)       : {WAPE:>6.1f}%  {'✅' if WAPE<15 else '❌'}")
    print(f"  WAPE  (excl outlier): {WAPE_excl:>6.1f}%  {'✅' if WAPE_excl<15 else '❌'}")
    print(f"\n  Weekday MADE : {MADE_wd:.1f}%  |  Weekend MADE : {MADE_we:.1f}%")
    print(f"  Std daily err: {daily_df['err_pct'].std():.1f}%")
    print(f"\n  Top 5 Worst Days:")
    for _, r in worst5.iterrows():
        print(f"    {str(r['tanggal'].date())}: pred={r['pred']:,} actual={r['actual']:,} err={r['err_pct']:+.1f}%")
    print(f"{'='*60}")
